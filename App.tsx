...
const App: React.FC = () => {
  ...
  // Updated defaults to Rahman and Indira
  const [boyName, setBoyName] = useState(() => localStorage.getItem('boyName') || "Rahman");
  const [girlName, setGirlName] = useState(() => localStorage.getItem('girlName') || "Indira");
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-rose-50 via-white to-pink-50 pb-20">
      ...
      <footer className="text-center py-8 text-gray-400 text-sm relative z-10">
        <p>Dibuat dengan cinta Rahman ❤️</p>
      </footer>
    </div>
  );
};
